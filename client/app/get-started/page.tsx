...
function RoleChoice({ href, icon, title, detail }: { href: string; icon: React.ReactNode; title: string; detail: string }) {
  return <Link href={href} className="group relative overflow-hidden rounded-3xl border border-[var(--color-border)] bg-white p-7 no-underline shadow-[var(--shadow-card)] transition-all duration-200 hover:-translate-y-1 hover:border-[var(--color-primary)] hover:shadow-[var(--shadow-elevated)] md:p-8"><div className="absolute right-0 top-0 size-32 rounded-full bg-[var(--color-primary-soft)] opacity-70 blur-2xl transition-transform duration-500 group-hover:scale-150" /><div className="relative"><div className="flex items-start justify-between"><span className="grid size-12 place-items-center rounded-2xl bg-[var(--color-primary-soft)] text-[var(--color-primary-dark)]">{icon}</span><span className="text-[var(--color-primary)] transition-transform duration-200 group-hover:translate-x-1"></span></div><h2 className="mt-8 text-2xl font-bold tracking-tight text-[var(--color-text-primary)]">{title}</h2><p className="mt-2 text-sm text-[var(--color-text-muted)]">{detail}</p><div className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-primary-dark)]">Continue <ArrowIcon /></div></div></Link>;
}

export default function GetStartedPage() {
  return <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[var(--color-page-bg)] px-6 py-12 text-[var(--color-text-primary)]"><div className="pointer-events-none absolute left-1/2 top-0 size-[36rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--color-primary-light)] opacity-50 blur-3xl" /><Link href="/" className="absolute left-6 top-6 inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-text-muted)] no-underline transition-colors hover:text-[var(--color-primary-dark)] md:left-10 md:top-8"><ArrowLeftIcon />Back</Link><div className="relative w-full max-w-4xl"><div className="flex justify-center"><Link href="/" className="inline-flex items-center gap-2.5 no-underline"><span className="grid size-9 place-items-center rounded-xl bg-[var(--color-primary)] text-white shadow-sm"><HeartPulseIcon /></span><span className="text-[17px] font-bold tracking-tight">MediRehab<span className="text-[var(--color-primary)]"> AI</span></span></Link></div><div className="mx-auto mt-16 max-w-2xl text-center md:mt-20"><div className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--color-primary)]">Get started</div><h1 className="mt-5 text-4xl font-bold leading-tight tracking-[-0.05em] md:text-6xl">Choose your workspace.</h1><p className="mt-5 text-base text-[var(--color-text-secondary)] md:text-lg">Select the view that fits your care journey.</p></div><div className="mt-12 grid gap-5 md:grid-cols-2"><RoleChoice href="/login/doctor" icon={<StethoscopeIcon />} title="Doctor" detail="Manage care plans and patient progress." /><RoleChoice href="/login/patient" icon={<PatientIcon />} title="Patient" detail="View exercises and track your sessions." /></div></div></main>;
}
